      ...
      <div className='container mx-auto px-8 md:px-16 lg:px-24'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
          {techArray.map((tech) => (
            <div 
              key={tech.id} 
              className='bg-gradient-to-r from-purple-600 to-blue-500 p-6 rounded-lg shadow-lg hover:scale-105 transition-transform duration-300'
            >
              <h2 className='text-2xl font-bold mb-4'>{tech.title}</h2>
              <p className='text-sm text-slate-100'>{tech.description}</p>
            </div>
          ))}
        </div>
      </div>
